...
// src/routes is included because the root route renders real user-facing UI (the
// 404 and error screens) — leaving it out is how English copy previously reached
// the HE/AR site unnoticed. src/components/ui is excluded on purpose: those are
// vendored shadcn primitives with no product copy.
const DIRS = ["src/pages", "src/components/site", "src/routes"];

// Generated by the router plugin — never hand-edited, so never a real violation.
...
// Walks nested route folders too (src/routes/$locale, src/routes/api, …).
function tsxFiles() {
  const out = [];
  const walk = (dir) => {
...
console.log(`✓ i18n check: no hard-coded UI strings in ${DIRS.join(", ")}`);
