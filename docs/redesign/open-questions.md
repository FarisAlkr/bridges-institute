...
| # | Item | Action |
|---|---|---|
| C1 | **Form submission** — does the apply/contact form actually POST anywhere, with a success state, on mobile? CV upload needs a real destination (endpoint / email / storage). | **CODE-COMPLETE, DELIVERY-UNVERIFIED.** Single canonical `/api/submit` handles both forms: server validation, CV type/size checks, then POSTs a **JSON payload to an email/automation webhook** (`SUBMISSIONS_WEBHOOK_URL`, set in Vercel — Make/Zapier/Resend). **CV is embedded as base64** so it always arrives (a real attachment / working link, not dropped); CV capped at **4 MB** to stay under Vercel's ~4.5 MB request-body limit. Success shows only on real delivery; `503 not_configured` otherwise (never fakes success). Tested vs a local receiver (422/503/200 + base64 CV present). **NOT closed until Faris sets the env var in Vercel and does ONE live end-to-end submit that lands in the inbox (fields + CV).** |
